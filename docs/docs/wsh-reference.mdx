...
List all SSH connections that have been made since Wave Terminal started, including connection status and remote host information.

---

## docker

The `docker` command runs Docker CLI commands from within Wave Terminal. When run locally, it passes commands directly to the Docker CLI. When run inside an SSH block, it creates a terminal block on the remote connection to execute the Docker command.

```sh
wsh docker [subcommand] [args...]
```

All subcommands pass arguments through to the Docker CLI. Available subcommands:
...
wsh docker logs --follow mycontainer

# Execute a command in a running container
wsh docker exec -it mycontainer /bin/bash

# Run docker compose
wsh docker compose up -d

# Stop a container
wsh docker stop mycontainer

# Remove a container
wsh docker rm mycontainer
```

When connected to a remote SSH session, `wsh docker` commands are executed on the remote machine by creating a terminal block on that connection. This allows you to manage Docker on remote servers without manually creating new blocks.
