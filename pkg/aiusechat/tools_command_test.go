...
func TestCheckCommandScopedAllowlist(t *testing.T) {
	tests := []struct {
		...
		{
			name: "allows task --list-all",
			command: `task --list-all`,
		},
		{
			name: "allows wsh input help",
			command: `wsh input --help`,
		},
		{
			name: "allows wsh input reply",
			command: `wsh input 83eb475a "Wave AI reply: Received your environment details." --enter`,
		},
	}
