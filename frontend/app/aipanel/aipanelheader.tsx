...
export const AIPanelHeader = memo(() => {
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center flex-shrink-0 whitespace-nowrap gap-2">
                {!inBuilder && (
                    <>
                        <div className="flex items-center text-sm whitespace-nowrap">
                            <span className="text-gray-300 @xs:hidden mr-1 text-[12px]">Auto</span>
                            <span className="text-gray-300 hidden @xs:inline mr-2 text-[12px]">Auto Approve</span>
                            <button
                                onClick={() => {
                                    model.setAutoApprove(!autoApprove);
                                }}
                                className={`relative inline-flex h-6 w-14 items-center rounded-full transition-colors cursor-pointer ${
                                    autoApprove ? "bg-green-600" : "bg-zinc-600"
                                }`}
                                title={`Auto Approve ${autoApprove ? "ON" : "OFF"}`}
                            >
                                <span
                                ...
                                <span
                                    className={`relative z-10 text-xs text-white transition-all ${
                                        autoApprove ? "ml-2.5 mr-6 text-left" : "ml-6 mr-1 text-right"
                                    }`}
                                >
                                    {autoApprove ? "ON" : "OFF"}
                                </span>
